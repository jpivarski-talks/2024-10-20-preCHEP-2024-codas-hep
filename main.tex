...
\vspace{0.25 cm}
\uncover<4->{Can be done in Jupyter, if necessary (usually isn't).}

\end{columns}
\end{frame}

\begin{frame}{Interactive exercises (Yana Osborne and me)}
\vspace{0.25 cm}
\large
\begin{columns}
\column{0.73\linewidth}
...
\small
\vspace{0.25 cm}
\uncover<1->{Each lesson has a slides (jupyterlab-deck)/workbook split for short problems and two long problems; only one requires HEP background.}

\small
\vspace{0.25 cm}
\uncover<2->{Students follow slides with their workbook, but read long problem descriptions on their own.}

\small
\vspace{0.25 cm}
\uncover<3->{We can present solutions if there's only one long problem (not here).}
\end{columns}
\end{frame}
